...
name: xhs-publish
description: |
  小红书（RedNote / Xiaohongshu / XHS）内容发布技能。
  支持图文、视频、长文、定时发布、标签和可见性设置。
  当用户要求向小红书、RedNote、Xiaohongshu、XHS 发笔记或上传内容时触发。
version: 1.0.0
metadata:
  openclaw:
    requires:
...
- **完成后收尾**：发布流程结束、取消或失败后，在最终答复前执行一次 `python scripts/cli.py cleanup`；分步发布等待用户确认期间不要提前收尾。
