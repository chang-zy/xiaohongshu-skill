...
- **完成后收尾**：每个工作流步骤完成后向用户报告进度，等待确认后继续；整个工作流结束、取消或失败后，在最终答复前执行一次 `python scripts/cli.py cleanup`。
...
| `browse-local` | 使用当前小红书会话自动定位浏览本地公开账号 |
...
| `cleanup` | 完整任务结束时清理自动化标签页并退出托管 Chrome |
...
- 竞品或热点样本包含视频笔记时，先用基础详情识别视频；若分析需要理解视频内容，必须提醒用户视频理解耗时较长且消耗较多模型额度，并在得到明确确认后才执行视频准备流程。未确认时只分析标题、封面、正文和互动数据，并明确说明没有读取视频内容。
- 需要本地样本时，优先使用 `browse-local`，不询问用户城市。它只依赖小红书当前会话应用“同城/附近”筛选，不能当作精确位置数据，也不能据此判断账号主体的身份、年龄、性别或外貌。
