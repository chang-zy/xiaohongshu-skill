...
- **完成后收尾**：搜索或浏览流程结束、取消或失败后，在最终答复前执行一次 `python scripts/cli.py cleanup`，再告知结果。
...
`--load-all-replies` 会在一级评论加载完成后，逐个展开所有“展开 N 条回复”按钮，
并在结果的 `commentLoadStatus` 中返回声明数量、已加载数量与完整性标记。
...
只有 `commentLoadStatus.complete: true` 时，才能称子回复已完整获取；否则应报告
